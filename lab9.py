import doctest
COUNTRY_NAME = 0
COUNTRY_NAME_AND_COUNTRY_POPULATION = 0
...
def count_evens_odds(filename: str) -> dict[str, int]:
    ...
    final_dictionary = {'Even': 0, 'Odd': 0}
    
    file_handle = open(f'{filename}', 'r', encoding='utf8')
    line = file_handle.readline()
    while line != '':
        if int(line) % 2 == 0:
            final_dictionary['Even'] += 1
        else:
            final_dictionary['Odd'] += 1
        line = file_handle.readline()
    file_handle.close()
...
def get_number_frequencies(filename: str) -> dict[int, int]:
    ...
    line = file_handle.readline()
    while line != '':
        if int(line) not in final_dictionary:
            final_dictionary[int(line)] = 1
        else:
            final_dictionary[int(line)] += 1
        line = file_handle.readline()
    file_handle.close()
...
def most_freq_number(filename: str) -> list[int]:
    ...
    all_numbers_dictionary = get_number_frequencies(filename)
    final_list = []
    largest_num = 0
    for index in all_numbers_dictionary:
        if all_numbers_dictionary[index] > largest_num:
            largest_num = all_numbers_dictionary[index]
...
def create_population_dictionaries(filename: str, granularity: int
    ...
    name_and_population = {}
    population_bin_and_countries = {}
    file_handle = open(f'{filename}', 'r', encoding='utf8')
    line = file_handle.readline()
    new_list = []
    while line != '':
        new_list = line.split(',')
        name_and_population[new_list[COUNTRY_NAME]] = int(new_list[COUNTRY_POPULATION])
        line = file_handle.readline()
    
    

    for index in name_and_population:
        if int(name_and_population[index]) // granularity not in population_bin_and_countries:
            population_bin_and_countries[int(name_and_population[index]) // granularity] = list((index,))
        else:
            # name_and_population[index] += index
            population_bin_and_countries[int(name_and_population[index]) // granularity].append(index)
    final_list = [name_and_population, population_bin_and_countries]
...
def population_analysis(filename: str, country: str, granularity: int
    ...
    # See the Function Specification in PrairieLearn for tips
    final_list = []
    population_granularity_tuple = create_population_dictionaries(f'{filename}', granularity)

    if not population_granularity_tuple or country not in population_granularity_tuple[COUNTRY_NAME_AND_COUNTRY_POPULATION]:
        final_list = []
    else:
        country_granularity = population_granularity_tuple[COUNTRY_NAME_AND_COUNTRY_POPULATION][country] // granularity
        for index in population_granularity_tuple[COUNTRY_NAME_AND_COUNTRY_POPULATION]:
            mini_list = []
            if population_granularity_tuple[COUNTRY_NAME_AND_COUNTRY_POPULATION][(index)] // granularity == country_granularity:
                mini_list.append(population_granularity_tuple[COUNTRY_NAME_AND_COUNTRY_POPULATION][index])
                mini_list.append(index)
                final_list.append(tuple(mini_list))
